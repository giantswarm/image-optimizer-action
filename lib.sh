...
_remote_commit() {
	...
	while read -r _src_mode dst_mode _src_sha dst_sha flag path; do
		...
		file_response=$(curl -s --fail -H "Authorization: token ${GITHUB_TOKEN}" \
			-d "$file_payload" \
			"https://api.github.com/repos/${GITHUB_REPOSITORY}/git/blobs")
		echo "{ \"mode\": \"${dst_mode}\", \"path\": \"${path}\", \"sha\": $(jq '.sha' <<<"$file_response")}" >>"$tmp_file"
	done < <(git diff-files)

	head_response="$(curl -s --fail -H "Authorization: token ${GITHUB_TOKEN}" \
		-X GET \
	...
	tree_response="$(curl -s --fail -H "Authorization: token ${GITHUB_TOKEN}" \
		-d "$tree_payload" \
	...
	commit_response="$(curl -s --fail -H "Authorization: token ${GITHUB_TOKEN}" \
		-d "$commit_payload" \
		"https://api.github.com/repos/${GITHUB_REPOSITORY}/git/commits")"

	update_branch_payload="{\"sha\": $(jq '.sha' <<<"$commit_response")}"
	# shellcheck disable=SC2034  # Unused variables left for readability
	update_branch_response="$(curl -s --fail -H "Authorization: token ${GITHUB_TOKEN}" \
